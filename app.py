...
try:
    ...
    st.error(f"⚠️ SISTEM TERHENTI: Artifak model (.pkl) bermasalah. Detail: {e}")
    st.stop()
...
with col1:
    ...
    umur = st.slider("Kelompok Usia Pendaftar (Binned)", 10, 90, 30, help="Perhatian: Evaluasi menunjukkan model bias terhadap pendaftar usia lanjut.")
...
with col2:
    ...
    sensitivitas = st.slider("Toleransi Manajemen Bank (%)", 1, 99, 50, help="Geser ke kiri untuk membuat AI lebih galak, ke kanan untuk lebih longgar.")
...
if tombol_evaluasi:
    with st.spinner('Menjalankan Audit AI HistGradientBoosting...'):
        ...
        # PERBAIKAN FATAL: Membangun matriks dengan 0.0 (Float) agar Pandas tidak crash
        ...
        # Logika One-Hot Encoding Dinamis
        dummy_mappings = {
        ...
        # Fitur Wow Factor Eksekutif: Peringatan Bias AI
        if umur >= 56:
            st.warning("""
